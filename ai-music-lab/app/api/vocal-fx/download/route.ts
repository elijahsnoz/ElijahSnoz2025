...
export async function GET(request: Request) {
  ...
  const backendUrl = new URL(`${VOCAL_FX_BACKEND_URL}/download/${encodeURIComponent(jobId)}`);
  ...
  if (!backendResponse.ok) {
    return NextResponse.json({ error: "That processed vocal could not be found." }, { status: backendResponse.status });
  }
